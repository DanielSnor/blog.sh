...
- **A language the engine has not been translated into can borrow another's interface.** `site.ui_language: { sk: cs }` builds a Slovak branch -- Slovak addresses, `<html lang="sk">`, its own `hreflang` -- with the engine's own words in Czech. Without it such a language is still refused, because inheriting English by accident is worse than being told.
