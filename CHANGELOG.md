...
- **The language switcher is a chip in the banner's corner, beside the light/dark button.** Which language you read the site in is a choice about this visit, like which mode you read it in -- it belongs where that choice already lives, not in a band under the menu.
- **A language with nothing for a post falls back to the nearest one you name.** `site.fallback` decides which, and the words and the link come from the same answer.
